/*global $:false, jQuery:true, Spine:true */
...
Contact.include({
	validate: function(){
		...
		if ( !this.contact_name ){return "Name is required";}
			 
		if ( !this.contact_phone ){return "Only numeric value | Required";}
			
		if ( !this.contact_message ) {return "It's required you to provide a message";}
...
var ContactControllerApp = Spine.Controller.sub({
	init: function(){
		Contact.bind("error", function(rec, msg) {
			return window.alert("Fail to submit - " + msg);
		});
	},
	...
	submitForm: function(e){
		...
		//contact.save();
		if (contact.save()) {
			window.alert(	"Name: " + contact.contact_name +
